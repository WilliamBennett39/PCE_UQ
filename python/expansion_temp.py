...
class coefficients_4d:
    ...
    def mote_carlo_sampler_pn(self, n):
        ...
        for ix, x in enumerate(tqdm(self.xlist)):
            self.mc_drive_pn_samples[ix] = monte_carlo_pn(sample, n, x, self.T0, self.kappa0, self.rho0, self.cv, self.omega, self.n, self.a1, self.a2, self.a3, self.a4, self.t, self.ximax, self.interp_t, self.interp_c, self.interp_k, self.interp_equi_spaced, self.interp_dx)
    ...
    # np.savetxt(name, sample_list)
        ...
        # dset1 = f.create_dataset(f"drive_He_{self.order_he}", data = self.c_He_drive)
        # dset1 = f.create_dataset(f"target_He_{self.order_he}", data = self.c_He_target)
        dset3 = f.create_dataset(f"drive", data = self.mc_drive_pn_samples)
        f.close()
